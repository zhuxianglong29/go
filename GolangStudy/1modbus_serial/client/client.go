...
func (mbus Mymodbus) serial(out chan<- []byte) {
	...
	for {
		dataserial, err := client.ReadHoldingRegisters(80, 0x34, 2)
		if err != nil {
			fmt.Println(err.Error())
		}
		logrus.Infoln("dataserial", dataserial)
		json_datamodbus, err := json.Marshal(dataserial) //dataserial是[]uint16格式
		out <- json_datamodbus
...
func (mbus Mymodbus) tcp(modbustcp_host string, modbustcp_port string, out chan<- []byte) {
...
func myclient(httpserver_host string, httpserver_port string, in <-chan []byte) {
	for {
		...
		logrus.Info("json", json_datemodbus)
